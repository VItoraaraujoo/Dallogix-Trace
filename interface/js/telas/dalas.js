...
export function dalaActions(store) {
  ...
  const actionRows = actions.length ? actions.map((item, index) => `<tr>
    ...
    <td><div class="dala-action-command"><strong>${esc(rotuloComando(item.comando))}</strong><code>${esc(item.comando)}</code></div></td><td><strong>${esc(item.rotulo)}</strong></td><td><span class="dala-color-swatch" data-color="${esc(item.cor)}"></span>${esc(item.cor[0] + item.cor.slice(1).toLowerCase())}</td><td>${Number(item.visivel) ? "Sim" : "Não"}</td><td>${esc(item.modo[0] + item.modo.slice(1).toLowerCase())}</td>
  ...
  const dalaName = dala.name || dala.equipment_code || "Dala";
  const dalaIdentifier = dala.equipment_code ? `<p class="muted">Identificador <code>${esc(dala.equipment_code)}</code></p>` : "";
  return `<div class="title-row with-actions dala-page-header"><div><span class="dala-page-kicker">Cadastros / Dalas</span><h2>Ações da Dala: ${esc(dalaName)}</h2>${dalaIdentifier}</div><div class="actions"><button class="button secondary page-back" data-action="back-dala" type="button">← Voltar</button>${canManage ? `<button class="button secondary" data-action="new-dala-trigger" type="button">Novo gatilho</button><button class="button primary" data-action="new-dala-action" type="button">Nova ação</button>` : ""}</div></div>
